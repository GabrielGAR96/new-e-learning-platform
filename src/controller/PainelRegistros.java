...
public class PainelRegistros {
    ...
    private <T> TableView<T> criarTabela(List<T> lista, Class<T> classe) {
        ...
        for (Field field : classe.getDeclaredFields()) {
            TableColumn<T, Object> coluna = new TableColumn<>(splitName(field));
        ...
        JFXButton editBtn = new JFXButton("Editar");
        JFXListView<JFXButton> btnList = new JFXListView<>();
        btnList.getItems().add(editBtn);
        btnList.getItems().add(deleteBtn);
        btnList.setMinHeight(80);
        btnList.setMinWidth(100);
        JFXPopup popup = new JFXPopup(btnList);
    ...
    private String splitName(Field field) {
        String[] names = field.getName().split("(?<!(^|[A-Z]))(?=[A-Z])|(?<!^)(?=[A-Z][a-z])");
    ...
    private <T> void editarLinha(TableView<T> tabela) {
        T elemento = tabela.getSelectionModel().getSelectedItem();
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("../view/painelAlterar.fxml"));
            AnchorPane ap =  fxmlLoader.load();
            PainelAlterar pac = fxmlLoader.getController();
            pac.setTabela(elemento);


            Stage stage = new Stage();
            stage.setTitle("Edição da tabela");
